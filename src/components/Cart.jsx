...
const Button = styled.button`
  ...
  cursor: pointer;
  border: ${(props) =>
    props.btnType === "conShoping" ? "2px solid black" : "none"};
  color: ${(props) => (props.btnType === "conShoping" ? "black" : "white")};
  background-color: ${(props) =>
    props.btnType === "conShoping" ? "white" : "black"};
...
function Cart() {
  const shiping = 10;
  const discount = 5;
  const [cartQty, setCartQty] = useState(UserCart);
  const [subTotal, setSubTotal] = useState(0);
  const navigate = useNavigate();
  useEffect(() => {
    window.scrollTo(0, 0);
    setSubTotal(cartQty.reduce(getSubtotal, 0));
  }, []);

  function handleCartItem(evt, id) {
    if (evt === "add") {
      setCartQty(
        cartQty.map((item) => {
          if (id === item.id) {
  ...
  return (
    <Container>
      <Header>
        <Left>
          <Button btnType="conShoping" onClick={() => navigate("/")}>
            CONTINUE SHOPPING
          </Button>
        </Left>
        <Center>
            ...
            return (
              <CartItem key={item.id}>
                ...
                <ProdInfo>
                  ...
                  <div style={{ margin: "30px 0" }}>
                    <ProdText>ID: </ProdText> <OrderId>{item.id}</OrderId>
                  </div>

                  <div style={{ margin: "30px 0" }}>
                    <OrderColor bg={item.color}></OrderColor>
                  </div>
                  <div style={{ margin: "30px 0" }}>
                    <ProdText>SIZE: </ProdText>{" "}
                    <OrderSize>{item.size}</OrderSize>
                  </div>
                </ProdInfo>
                <CartTotal>
                  <QtyOptions>
        ...
        <CartSummary>
          <SummaryCard>
            ...
            <OrderSumaryItem>
              ...
              <ShippingCost>$ {shiping}</ShippingCost>
            </OrderSumaryItem>
            <OrderSumaryItem>
              <Text>Shipping Estimate</Text>
            ...
            <OrderSumaryItem>
              ...
              <Total>$ {subTotal + shiping - discount}</Total>
            </OrderSumaryItem>
            <ButtonWrapper>
              {" "}
              <Button lgbtn="lg">CHECKOUT NOW</Button>
            </ButtonWrapper>
          </SummaryCard>
        </CartSummary>
      </Wrapper>
